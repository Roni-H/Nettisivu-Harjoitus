    ...
    <section class="about-section text-center" id="about">
        <div class="container px-4 px-lg-5">
            <div class="row gx-4 gx-lg-5 justify-content-center">
                <div class="col-lg-8">
                    <h1 class="text-white mb-12">Database tiedot</h1>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="container px-4 px-lg-5">
            <div class="row gx-4 gx-lg-5 justify-content-center">
                ...
                <div>
                    ...
                    $sql = "SELECT * FROM lomake";
                    $result = $conn->query($sql);

                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo '<span style="color:#FFFFFF;text-align:center;">
                            ' . $row["ID"] . '.
                            </span>';
                            echo '<span style="color:#FFFFFF;text-align:center;">
                            Ajopäivämäärä : ' . $row["Paivamaara"] . '<br>
                            </span>';
                            echo '<span style="color:#FFFFFF;text-align:center;">
                            Ajetut kilometrit : ' . $row["Kilometrit"] . '<br>
                            </span>';
                            echo '<span style="color:#FFFFFF;text-align:center;">
                            Kulutettujen hiilidioksidipäästöjen määrä : ' . $row["Paastot"] . '<br> <br>
                            </span>';
                        }
                    } else {
                        echo '<script>alert("Ei tuloksia")</script>';
                    }
                    $conn->close();
                    ?>
                </div>
